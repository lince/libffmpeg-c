...
double parse_number_or_die(const char *context, const char *numstr, int type, double min, double max)
{
    ...
    sprintf(error_str, error, context, numstr, min, max);
...
int64_t parse_time_or_die(const char *context, const char *timestr, int is_duration)
{
    ...
    if (av_parse_time(&us, timestr, is_duration) < 0) {
        sprintf(error_str, "Invalid %s specification for %s: %s\n",
...
//TODO: Verificar direito o lance dos erros
int opt_default(const char *opt, const char *arg){
    int type;
    int ret= FFMpeg_SUCCESS;
    ...
    if (oformat) {
    	printf("Encontramos o formato de saida.\n");
        goto out;
    }
    ...
    if (iformat) {
    	printf("Encontramos o formato de entrada.\n");
        goto out;
    }
    ...
    if (o && ret < 0) {
        sprintf(error_str, "Invalid value '%s' for option '%s'\n", arg, opt);
        return FFMpeg_ERROR;
    }
    if (!o) {
        fprintf(error_str, "Unrecognized option '%s'\n", opt);
        return FFMpeg_ERROR;
    }
...
int opt_timelimit(long int limit)
{
#if HAVE_SETRLIMIT
    int lim = limit;
    ...
    if (setrlimit(RLIMIT_CPU, &rl))
        perror("setrlimit");
#else
    sprintf(error_str, "Set timing limit is not implemented on this OS.");
    return FFMpeg_ERROR;
#endif
    return FFMpeg_SUCCESS;
}
...
void sprint_error(char* error_str, const char *filename, int err) {
    char errbuf[128];
    const char *errbuf_ptr = errbuf;
