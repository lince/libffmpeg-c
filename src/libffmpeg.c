...
#include "../deps/ffmpeg/libavcore/parseutils.h"
#include "../deps/ffmpeg/libavutil/colorspace.h"
#include <libavutil/fifo.h>
#include <libavutil/pixdesc.h>
#include <libavutil/avstring.h>
#include "../deps/ffmpeg/libavutil/libm.h"
#include "../deps/ffmpeg/libavformat/os_support.h"

#if CONFIG_AVFILTER
# include "../deps/ffmpeg/libavfilter/avfilter.h"
# include "../deps/ffmpeg/libavfilter/avfiltergraph.h"
# include "../deps/ffmpeg/libavfilter/graphparser.h"
# include "../deps/ffmpeg/libavfilter/vsrc_buffer.h"
#endif
...
static int grabingFrame = 0;
static void* currentFrame = NULL;
...
static void codecAux(int *pstream_copy, char **pcodec_name,
...
void FFMpeg_setStreamId(char* arg) {
    ...
    *p++ = '\0';
    idx = parse_number_or_die(NULL, idx_str, OPT_INT, 0, MAX_STREAMS-1);
    streamid_map[idx] = parse_number_or_die(NULL, p, OPT_INT, 0, INT_MAX);
}
...
void FFMpeg_reset() {
	...
	printf("Depois de desalocar o last_asked_format ()\n" );
	...
	printf("Depois do segundo for ()" );
	nb_input_files = 0;
	nb_icodecs =0;
		//TODO: Talvez seja necessário desalocar os codecs, talvez não.
}
...
void FFMpeg_setVerbose(int verboseLevel) {
	if (verboseLevel > -10 && verboseLevel < 10) {
		verbose = verboseLevel;
	} else {
		fprintf(stderr, "Incorrect value for %s: %d\n", "verbose", verboseLevel);
	}
}
...
void FFMpeg_setFrameCrop(int top, int bottom, int left, int right) {
	if (top < 0) {
		...
		/*throw OptionException(
				"\n\tFFrapper::setFrameCrop().\n"
				"\tTrying to crop a negative value on top.");*/
	} else if (top >= frame_height) {
		fprintf(stderr, "Incorrect value for %s: %d\n", "frame crop top", top);
		//todo: implementar erros.
		/*throw OptionException(
				"\n\tFFrapper::setFrameCrop().\n"
				"\tTrying to crop a value bigger than the frame height"
				" on top.");*/
	}
	frame_topBand = top;
	...
	if (bottom < 0) {
		...
		/*throw OptionException(
				"\n\tFFrapper::setFrameCrop().\n"
				"\tTrying to crop a negative value on bottom.");*/
	} else if (bottom >= frame_height) {
		fprintf(stderr, "Incorrect value for %s: %d\n", "frame crop bottom", bottom);
		//todo: implementar erros.
		/*throw OptionException(
				"\n\tFFrapper::setFrameCrop().\n"
	...
	if (left < 0) {
		...
		/*throw OptionException(
				"\n\tFFrapper::setFrameCrop().\n"
				"\tTrying to crop a negative value on left.");*/
	} else if (left >= frame_width) {
		fprintf(stderr, "Incorrect value for %s: %d\n", "frame crop left", left);
		//todo: implementar erros.
		/*throw OptionException(
				"\n\tFFrapper::setFrameCrop().\n"
				"\tTrying to crop a value bigger than the frame width"
				" on left.");*/
	}
	frame_leftBand = left;
	...
	if (right < 0) {
		...
		/*throw OptionException(
				"\n\tFFrapper::setFrameCrop().\n"
				"\tTrying to crop a negative value on right.");*/
	} else if (right >= frame_width) {
		fprintf(stderr, "Incorrect value for %s: %d\n", "frame crop right", right);
		//todo: implementar erros.
		/*throw OptionException(
				"\n\tFFrapper::setFrameCrop().\n"
...
void FFMpeg_setMetadata1(char* key, char* value) {
	...
	metadata[metadata_count-1].key  = av_strdup(key);
	metadata[metadata_count-1].value= av_strdup(value);
}
...
void FFMpeg_setThreadCount(int number) {
	if (number > 0) {
		...
		fprintf(stderr, "Trying to set a negative value to thread count");
		//todo: erro ffmpeg_exit(1);
		/*throw OptionException(
				"\n\tFFrapper::setThreadCount().\n"
				"\tTrying to set a negative value to thread count.");*/
	}
#if !HAVE_THREADS
    if (verbose >= 0) {
    	LoggerUtil_warning(logger, "Warning: not compiled with thread support, using thread emulation");
    }
#endif
}
...
void FFMpeg_setAudioRate(int arate) {
	if (arate > 0) {
		...
		fprintf(stderr, "Trying to set a negative value to audio rate");
		//todo: erro ffmpeg_exit(1);
		/*throw OptionException(
				"\n\tFFrapper::setAudioRate().\n"
				"\tTrying to set a negative value to audio rate.");*/
	}
}
...
void FFMpeg_setAudioChannels(int achannel) {
	if (achannel > 0) {
		...
		fprintf(stderr, "Trying to set a negative value to audio channels number");
		//todo: erro ffmpeg_exit(1);
		/*throw OptionException(
				"\n\tFFrapper::setAudioChannels().\n"
				"\tTrying to set a negative value to audio channels number.");*/
	}
}
...
void FFMpeg_setVideoChannel(int vchannel) {
	if (vchannel > 0) {
		...
		fprintf(stderr, "Trying to set a negative value to video channels number");
		//todo: erro ffmpeg_exit(1);
		/*throw OptionException(
				"\n\tFFrapper::setVideoChannel().\n"
				"\tTrying to set a negative value to video channels number.");*/
	}
}
...
void FFMpeg_setAudioCodec(char* arg) {
	codecAux(&audio_stream_copy, &audio_codec_name, AVMEDIA_TYPE_AUDIO, arg);
}
...
void FFMpeg_setVideoCodec(char* arg) {
	codecAux(&video_stream_copy, &video_codec_name, AVMEDIA_TYPE_VIDEO, arg);
}

void FFMpeg_setSubtitleCodec(char* arg) {
	codecAux(&subtitle_stream_copy, &subtitle_codec_name, AVMEDIA_TYPE_SUBTITLE, arg);
}
...
void FFMpeg_setRecordingTime(char* time) {
	recording_time = parse_time_or_die(NULL, time, 1);
}
...
void FFMpeg_setRecTimestamp(char* time) {
	recording_timestamp = parse_time_or_die(NULL, time, 0) / 1000000;
}

void FFMpeg_setInputTSOffset(char* time) {
	input_ts_offset = parse_time_or_die(NULL, time, 1);
}
...
static int presetAux(const char *opt, const char *arg) {
    FILE *f=NULL;
