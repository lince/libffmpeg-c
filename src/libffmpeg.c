...
#include "../deps/ffmpeg/libavcodec/audioconvert.h"
#include <libavutil/audioconvert.h>
#include <libavutil/parseutils.h>
#include <libavutil/samplefmt.h>
#include "../deps/ffmpeg/libavutil/colorspace.h"
#include <libavutil/fifo.h>
#include <libavutil/intreadwrite.h>
#include <libavutil/dict.h>
#include <libavutil/pixdesc.h>
#include <libavutil/avstring.h>
#include "../deps/ffmpeg/libavutil/libm.h"
#include "libavformat/os_support.h"

#include "../deps/ffmpeg/libavformat/ffm.h" // not public API
...
#define MAKE_SFMT_PAIR(a,b) ((a)+AV_SAMPLE_FMT_NB*(b))
    if (!ost->audio_resample && dec->sample_fmt!=enc->sample_fmt &&
        ...
        if (!ost->reformat_ctx) {
        	sprintf(error_str, "Cannot convert %s sample format to %s sample format\n",
...
static int do_video_stats(AVFormatContext *os, AVOutputStream *ost,
                           int frame_size)
{
    AVCodecContext *enc;
    int frame_number;
...
static int parse_forced_key_frames(char *kf, AVOutputStream *ost,
...
static int parse_meta_type(char *arg, char *type, int *index, char **endptr)
{
    *endptr = arg;
    if (*arg == ',') {
        *type = *(++arg);
...
static int new_data_stream(AVFormatContext *oc, int file_idx)
{
    AVStream *st;
    AVCodec *codec=NULL;
...
void FFMpeg_reset(int line) {
    ...
    /* TODO: Check for reinitialize options */
    uninit_opts();
    init_opts();
    ...
    //TODO: Check for this AVRational default value;
    //static AVRational frame_rate;
    video_qscale = 0;
    av_free(video_rc_override_string);
    ...
    av_free(vfilters);
    vfilters = NULL;
    //TODO: check for avfilter_uninit() need
    //avfilter_uninit();
#endif
    ...
    //TODO: check the AVDictionary delete
    av_free(metadata);
    metadata = NULL;
    ...
    limit_filesize = 0;
    force_fps = 0;
    av_free(forced_key_frames);
    forced_key_frames = NULL;

    dts_delta_threshold = 10;
    timer_start = 0;

    //TODO: check need to dealloc video_bitstream_filters.
    video_bitstream_filters=NULL;
    audio_bitstream_filters=NULL;
    subtitle_bitstream_filters=NULL;
...
int FFMpeg_setInputFile(const char* filename) {
	...
	for(i=0;i<ic->nb_streams;i++) {
		...
		case AVMEDIA_TYPE_UNKNOWN:
			break;
		default:
			//TODO: Check for fatal error option
			abort();
			break;
		}
	}
...
int FFMpeg_setRecordingTime2(long int time) {
	if (time < 0) {
		sprintf(error_str, "Invalid value for duration on FFMpeg_setRecordingTime2");
		return FFMpeg_ERROR;
	}
	recording_time = time;
	return FFMpeg_SUCCESS;
...
int FFMpeg_setInputTSOffset1(char* time) {
    input_ts_offset = parse_time_or_die("FFMpeg_setInputTSOffset1", time, 1);
    return error_number;
}

int FFMpeg_setInputTSOffset2(unsigned long int time) {
    input_ts_offset = time;
    return FFMpeg_SUCCESS;
}
...
int FFMpeg_setInputTSScale(const char* arg) {
    ...
    input_files_ts_scale[nb_input_files] = grow_array(input_files_ts_scale[nb_input_files],
    		sizeof(*input_files_ts_scale[nb_input_files]), &nb_input_files_ts_scale[nb_input_files], stream + 1);
...
int FFMpeg_setRecordingTimestamp1(char* time) {
    recording_timestamp = parse_time_or_die("FFMpeg_setRecordingTimestamp1", time, 0) / 1000000;
    return error_number;
}

int FFMpeg_setRecordingTimestamp2(unsigned long int  time) {
    recording_timestamp = time / 1000000;
    return FFMpeg_SUCCESS;
}
...
int FFMpeg_setDataFramesToRecord(int value) {
	max_frames[AVMEDIA_TYPE_DATA] = value;
	return FFMpeg_SUCCESS;
}

int FFMpeg_setDoBenchMark(int boolean) {
	do_benchmark = boolean;
	return FFMpeg_SUCCESS;
}

int FFMpeg_setTimeLimit(long int time) {
	return opt_timelimit(time);
}
...
int FFMpeg_setTarget(char* arg) {
	...
	if(!strcmp(arg, "vcd")) {
		FFMpeg_setVideoCodec("mpeg1video");
		FFMpeg_setAudioCodec("mp2");
		FFMpeg_setFormat("vcd");

		FFMpeg_setFrameSize1(norm == PAL ? "352x288" : "352x240");
		FFMpeg_setFramerate(frame_rates[norm]);
		...
		FFMpeg_setVideoCodec("mpeg2video");
		FFMpeg_setAudioCodec("mp2");
		FFMpeg_setFormat("svcd");

		FFMpeg_setFrameSize1(norm == PAL ? "480x576" : "480x480");
		FFMpeg_setFramerate(frame_rates[norm]);
		FFMpeg_setPixelFormat("yuv420p");
		opt_default("g", norm == PAL ? "15" : "18");
		...
		FFMpeg_setVideoCodec("mpeg2video");
		FFMpeg_setAudioCodec("ac3");
		FFMpeg_setFormat("dvd");

		FFMpeg_setFrameSize1(norm == PAL ? "720x576" : "720x480");
		FFMpeg_setFramerate(frame_rates[norm]);
		FFMpeg_setPixelFormat("yuv420p");
		opt_default("g", norm == PAL ? "15" : "18");
		...
		FFMpeg_setFrameSize1(norm == PAL ? "720x576" : "720x480");
		FFMpeg_setPixelFormat(!strncmp(arg, "dv50", 4) ? "yuv422p" :
		...
		sprintf(error_str, "Unknown target: %s", arg);
		return FFMpeg_ERROR;
	}
	return FFMpeg_SUCCESS;
}
...
int FFMpeg_setShortestInput(int boolean) {
	opt_shortest = boolean;
	return FFMpeg_SUCCESS;
}

int FFMpeg_setDTSDeltaThreshould(float number) {
	dts_delta_threshold = number;
	return FFMpeg_SUCCESS;
}

int FFMpeg_setProgramId(int number) {
	opt_programid = number;
	return FFMpeg_SUCCESS;
}

int FFMpeg_setCopyInitialNonKeyframes(int boolean) {
	copy_initial_nonkeyframes = boolean;
	return FFMpeg_SUCCESS;
}
...
int FFMPeg_setVideoMaxFrames(int number) {
	max_frames[AVMEDIA_TYPE_VIDEO] = number;
	return FFMpeg_SUCCESS;
}
...
int FFMpeg_setFrameSize1(char* arg) {
    if (av_parse_video_size(&frame_width, &frame_height, arg) < 0) {
        sprintf(error_str, "Incorrect frame size.");
        return FFMpeg_ERROR;
    }
    return FFMpeg_SUCCESS;
}
...
int FFMpeg_setAspectRatio(char* arg) {
    ...
    if (!ar) {
        sprintf(error_str, "Incorrect aspect ratio specification.");
        return FFMpeg_ERROR;
    }
...
int FFMpeg_setPixelFormat(char* arg) {
	...
	if (frame_pix_fmt == PIX_FMT_NONE) {
		sprintf(error_str, "Unknown pixel format requested: %s\n", arg);
		return FFMpeg_ERROR;
	}
...
int FFMPeg_setBitsPerRawSample(int number) {
	if (number < 0) {
		...
		frame_bits_per_raw_sample = number;
	}
	return FFMpeg_SUCCESS;
}
...
int FFMpeg_setDisableVideo(int boolean) {
	video_disable = boolean;
	return FFMpeg_SUCCESS;
}

int FFMpeg_setVideoDiscard(int boolean) {
	video_discard = boolean;
	return FFMpeg_SUCCESS;
}

int FFMpeg_setVideoQScale(float qscale) {
    if (qscale <= 0 || qscale > 255) {
        sprintf(error_str, "qscale must be > 0.0 and <= 255\n");
        return FFMpeg_ERROR;
    }
    video_qscale = qscale;
    return FFMpeg_SUCCESS;
...
int FFMpeg_setVideoCodec(char* arg) {
    ...
    pstream_copy = &video_stream_copy;
    pcodec_name = &video_codec_name;
...
int FFMpeg_setVideoSameQuality(int boolean) {
	same_quality = boolean;
	return FFMpeg_SUCCESS;
}

int FFMpeg_setPassNumber(int number) {
	if (number != 1 && number != 2) {
		sprintf(error_str, "You should choose 1 or 2 pass.");
		return FFMpeg_ERROR;
	}
    do_pass = number;
    return FFMpeg_SUCCESS;
...
#if CONFIG_AVFILTER
	av_free (vfilters);
	vfilters = av_strdup (arg);
	return FFMpeg_SUCCESS;
#else
	sprintf(error_str, "Filters aren't enable in this instance.");
	return FFMpeg_ERROR;
#endif
}
...
int FFMpeg_setTopFieldFirst(char* arg) {
    top_field_first = parse_number_or_die("top", arg, OPT_INT, 0, 1);
    if (error_number == FFMpeg_ERROR) {
    	return error_number;
    }
    opt_default("top", arg);
    return error_number;
...
int FFMpeg_setVideoTag(char* arg) {
    char *tail;
    uint32_t *codec_tag = &video_codec_tag;
...
int FFMpeg_setStreamId1(char* arg) {
    ...
    *p++ = '\0';
    idx = parse_number_or_die("FFMpeg_setStreamId1", idx_str, OPT_INT, 0, MAX_STREAMS-1);
    if (error_number == FFMpeg_ERROR) {
    	return FFMpeg_ERROR;
    }
    streamid_map = grow_array(streamid_map, sizeof(*streamid_map), &nb_streamid_map, idx+1);
    streamid_map[idx] = parse_number_or_die("FFMpeg_setStreamId1", p, OPT_INT, 0, INT_MAX);
    return error_number;
}
...
int FFMpeg_setStreamId2(int streamIndex, int value) {
    ...
    streamid_map = grow_array(streamid_map, sizeof(*streamid_map), &nb_streamid_map, streamIndex+1);
    streamid_map[streamIndex] = value;
    return FFMpeg_SUCCESS;
}

int FFMpeg_setForceKeyFrame(char* arg) {
	av_free (forced_key_frames);
	forced_key_frames = av_strdup (arg);
	return FFMpeg_SUCCESS;
}
...
int FFMPeg_setAudioMaxFrames(int number) {
	max_frames[AVMEDIA_TYPE_AUDIO] = number;
	return FFMpeg_SUCCESS;
}
...
int FFMpeg_setAudioCodec(char* arg) {
    ...
    pstream_copy = &audio_stream_copy;
    pcodec_name = &audio_codec_name;
...
int FFMpeg_setAudioTag(char* arg) {
    char *tail;
    uint32_t *codec_tag = &audio_codec_tag;
...
int FFMpeg_setAudioSampleFormat(char* arg) {
	...
	if (audio_sample_fmt == AV_SAMPLE_FMT_NONE) {
		sprintf(error_str, "Invalid sample format %s.", arg);
		return FFMpeg_ERROR;
	}
	return FFMpeg_SUCCESS;
}
...
int FFMpeg_setSubtitleCodec(char* arg) {
    ...
    pstream_copy = &subtitle_stream_copy;
    pcodec_name = &subtitle_codec_name;
...
int FFMpeg_newSubtitleStream() {
    ...
    new_subtitle_stream(oc, file_idx);
...
int FFMpeg_setSubtitleLanguage(char* arg) {
	av_free (subtitle_language);
	subtitle_language = av_strdup (arg);
	return FFMpeg_SUCCESS;
}

int FFMpeg_setSubtitleTag(char* arg) {
    char *tail;
    uint32_t *codec_tag = &subtitle_codec_tag;
...
int FFMpeg_setSubtitleBitstreamFilter(char* arg) {
    ...
    bsfp= &subtitle_bitstream_filters;
...
int FFMpeg_setAudioPreset(char* arg) {
	...
	char *codec_name =  audio_codec_name;
	...
	while(!feof(f)){
		...
		if ( !strcmp(tmp, "acodec") ) {
			FFMpeg_setAudioCodec(tmp2);
		} else if ( !strcmp(tmp, "vcodec") ) {
			FFMpeg_setVideoCodec(tmp2);
		} else if ( !strcmp(tmp, "scodec") ) {
			FFMpeg_setSubtitleCodec(tmp2);
...
int FFMpeg_setVideoPreset(char* arg) {
	FILE *f=NULL;
	char filename[1000], tmp[1000], tmp2[1000], line[1000];
	char *codec_name =  video_codec_name;
...
int FFMpeg_setSubtitlePreset(char* arg) {
	FILE *f=NULL;
	char filename[1000], tmp[1000], tmp2[1000], line[1000];
	char *codec_name =  subtitle_codec_name;
...
int FFMpeg_setDataCodec(char* arg) {
    ...
    pstream_copy = &data_stream_copy;
    pcodec_name = &data_codec_name;
